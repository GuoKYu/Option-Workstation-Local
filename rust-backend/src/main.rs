...
/// Query parameters Longbridge redirects back with after the user authorizes
/// (or denies) the cloud OAuth flow.
#[derive(Deserialize)]
...
/// Server-side OAuth redirect target. Longbridge sends the browser here with
/// `?code=...&state=...`; we exchange the code for a token and finish the flow.
async fn oauth_callback(
    State(state): State<AppState>,
    Query(params): Query<OAuthCallbackQuery>,
) -> Response {
    // Longbridge may redirect back with an error (user denied, or the
    // redirect_uri is not whitelisted in the Longbridge app) instead of a code.
    if let Some(error) = &params.error {
        let description = params.error_description.clone().unwrap_or_default();
        tracing::warn!(%error, %description, "Longbridge OAuth callback returned an error");
        return Html::from(format!(
            "<!doctype html><html lang=\"zh\"><head><meta charset=\"utf-8\">\
             <title>OAuth 失败</title></head><body style=\"font-family:sans-serif;padding:24px\">\
             <h3>Longbridge OAuth 授权失败</h3>\
             <p>错误：{error}</p>\
             <p>{description}</p>\
             <p>请关闭此窗口，返回 OptionWorkstation 重试。常见原因：长桥应用未把本回调地址加入 \
             redirect_uri 白名单。</p>\
             <script>setTimeout(function(){{window.close();}}, 9000);</script>\
             </body></html>"
        ))
        .into_response();
    }
    ...
    let code = match params.code {
        ...
        _ => {
            return Html::from(
                "<!doctype html><html lang=\"zh\"><head><meta charset=\"utf-8\">\
                 <title>OAuth 失败</title></head><body style=\"font-family:sans-serif;padding:24px\">\
                 <h3>缺少授权码</h3><p>回调未携带 code 参数，请关闭窗口重试。</p>\
    ...
    match state.live.handle_oauth_callback(code, state_param).await {
        Ok(_) => Html::from(
            "<!doctype html><html lang=\"zh\"><head><meta charset=\"utf-8\">\
             <title>授权成功</title></head><body style=\"font-family:sans-serif;padding:24px\">\
             <h3>Longbridge OAuth 授权成功 ✓</h3>\
             <p>可以关闭此窗口并返回 OptionWorkstation 了。</p>\
             <script>window.close();</script></body></html>",
        )
        .into_response(),
        Err(error) => {
            let detail = format!("{error:#}");
            tracing::warn!(error = %detail, "OAuth callback processing failed");
            Html::from(format!(
                "<!doctype html><html lang=\"zh\"><head><meta charset=\"utf-8\">\
                 <title>授权失败</title></head><body style=\"font-family:sans-serif;padding:24px\">\
                 <h3>令牌交换失败</h3><p>{detail}</p>\
                 <p>请关闭此窗口重试。</p>\
                 <script>setTimeout(function(){{window.close();}}, 9000);</script>\
                 </body></html>"
            ))
...
/// Diagnostics: probe whether this container can reach Longbridge's API hosts.
/// Used to distinguish "OAuth 502 = network blocked" from "OAuth 502 = bad credentials".
async fn debug_connectivity() -> Json<Value> {
    async fn tcp_ok(host: &str) -> bool {
        tokio::net::TcpStream::connect(host).await.is_ok()
    }
    let com = tcp_ok("openapi.longbridge.com:443").await;
    let cn = tcp_ok("openapi.longbridge.cn:443").await;
    Json(json!({
        "openapi_longbridge_com_443": com,
        "openapi_longbridge_cn_443": cn,
    }))
}
...
fn app(state: AppState, frontend_dist: PathBuf) -> Router {
    ...
    Router::new()
        ...
        .route("/api/debug/connectivity", get(debug_connectivity))
