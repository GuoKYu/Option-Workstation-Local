...
struct OAuthFlowState {
    ...
    /// Cloud (server-side) OAuth client awaiting its redirect callback.
    pending_oauth: Option<OAuth>,
}

impl Default for OAuthFlowState {
    fn default() -> Self {
        Self {
            status: OAuthStatus::default(),
            task: None,
            pending_oauth: None,
        }
    }
...
impl LiveManager {
    ...
    pub async fn start_oauth(self: &Arc<Self>, client_id: String) -> anyhow::Result<OAuthStatus> {
        ...
        // Cloud (server-side) OAuth: drive the flow via a public redirect URI.
        // Set env OW_OAUTH_REDIRECT_BASE=https://<public-host>; Longbridge must
        // whitelist `{base}/api/oauth/callback` as an allowed redirect URI.
        if let Ok(base) = std::env::var("OW_OAUTH_REDIRECT_BASE")
            && !base.trim().is_empty()
        {
            ...
            let oauth = OAuthBuilder::new(client_id.clone())
                .redirect_base(base)
                .token_storage(storage)
                .build(|_url: &str| {})
                .await
                .map_err(|error| anyhow!("Longbridge OAuth 初始化失败: {error}"))?;
            let (authorization_url, _csrf_state) = oauth
                .authorization_url()
                .await
                .map_err(|error| anyhow!("生成授权链接失败: {error}"))?;
    ...
    /// Handle the server-side (cloud) OAuth callback. Longbridge redirects the
    /// browser here with `?code=...&state=...`; we exchange the code for a
    /// token (validating the CSRF `state`) and finish the flow.
    pub async fn handle_oauth_callback(
        ...
        let (flow_id, oauth) = {
            let mut flow = self.oauth_flow.lock().await;
            let oauth = flow.pending_oauth.take().ok_or_else(|| {
                anyhow!("没有进行中的云端 OAuth 流程，请先在设置中点击「开始 OAuth 授权」")
            })?;
            let flow_id = flow.status.flow_id.clone().unwrap_or_default();
            (flow_id, oauth)
        };
        let result = oauth.exchange_code(code, state).await;
        Arc::clone(self).finish_oauth(flow_id, result).await;
        Ok(self.oauth_status().await)
    }
