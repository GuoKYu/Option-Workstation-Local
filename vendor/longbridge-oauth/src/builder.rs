...
pub struct OAuthBuilder {
    ...
    /// Public base URL for server-side (cloud) OAuth callbacks, e.g.
    /// `https://example.com`. When `Some`, the flow does not start a local
    /// callback server and callers must drive it via
    /// [`OAuth::authorization_url`](crate::OAuth::authorization_url) and
    /// [`OAuth::exchange_code`](crate::OAuth::exchange_code). The redirect URI
    /// becomes `{base}/api/oauth/callback`.
...
impl OAuthBuilder {
    ...
    /// Configure the public base URL for server-side (cloud) OAuth.
    ///
    /// When set, [`build`](OAuthBuilder::build) returns an [`OAuth`] without
    /// starting a local callback server. Drive the flow manually:
    ///
    /// 1. `let (url, _csrf) = oauth.authorization_url().await?;`
    /// 2. Redirect the user's browser to `url`.
    /// 3. On callback, `let oauth = oauth.exchange_code(code, state).await?;`
    ///
    /// The redirect URI registered with Longbridge must be
    /// `{base}/api/oauth/callback`.
    #[must_use]
    pub fn redirect_base(mut self, base: impl Into<String>) -> Self {
        self.redirect_base = Some(base.into().trim_end_matches('/').to_string());
        self
    }
    ...
    pub async fn build(self, open_url: impl Fn(&str)) -> OAuthResult<OAuth> {
        ...
        let inner = Arc::new(OAuthInner {
            ...
            redirect_base: self.redirect_base.clone(),
            token: tokio::sync::Mutex::new(None),
            pending_csrf: tokio::sync::Mutex::new(None),
        });
        let oauth = OAuth(inner);

        // Server-side (cloud) mode: the caller drives the authorization flow
        // via `authorization_url` / `exchange_code` and supplies a public
        // redirect URI. Do NOT start a local callback server here.
        if self.redirect_base.is_some() {
            return Ok(oauth);
        }
